...
def get_args_parser():
    ...
    parser.add_argument('--resume', default='671mot17_crowdhuman_mot17.pth', help='resume from checkpoint')
...
def main(args):
    ...
    while res:
        count += 1
        if (not run_all_frame) and (count <= start_frame):
            res, img = cap.read()
            continue
        resized_img, nh, nw = resize(img, size=540, max_size=960)
